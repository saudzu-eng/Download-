...
def progress_hook(d, status_msg, chat_id):
    if d['status'] == 'downloading':
        ...
        if total > 0:
            ...
            text = f"📥 **جاري سحب المقطع بأعلى جودة HD أصلية...**\n\n{bar} `{percent:.1f}%`\n⚡ السرعة: `{speed_mb:.2f} MB/s`"
...
def send_welcome(message):
    bot.reply_to(message, "🔥 **مرحباً بك في بوت تيك توك المطور لأعلى جودة HD!**\n\nأرسل الرابط الآن وسيقوم البوت بسحب النسخة الأصلية المرفوعة بدون أي ضغط وبدون علامة مائية 🎬", parse_mode='Markdown')
...
def handle_download(message):
    ...
    status_msg = bot.reply_to(message, "🔍 جاري فحص الرابط واستخراج الجودة الأصلية المستقرة...", parse_mode='Markdown')
